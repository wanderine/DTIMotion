...
% Load DTI data
data = load_untouch_nii('data/b1000.nii');
...
new_file.hdr.dime.dim = [3 sy sx sz st 1 1 1];
new_file.hdr.dime.datatype = 16;
new_file.hdr.dime.bitpix = 32;
new_file.img = single(generated_DTI_volumes);
filename = ['data/b1000_with_random_motion.nii'];
save_untouch_nii(new_file,filename);

%%

filename='b1000_true_motion_parameters';
save(filename,'x_translations','y_translations','z_translations','x_rotations','y_rotations','z_translations');
